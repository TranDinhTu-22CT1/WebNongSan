  ...
  return (
    <div className="shipping-page-container">
      <style>{`
        ...
        .form-control-green:focus, .form-select-green:focus { background-color: #1E3923; border-color: #95d5b2; color: #fff; box-shadow: 0 0 0 0.25rem rgba(118, 200, 147, 0.25); }
        .text-id { color: #ffd166; font-weight: 700; }
        .text-sub { color: #95d5b2; font-size: 0.85rem; }
        .mobile-card { background-color: #1E3923; border: 1px solid #558b6e; border-radius: 8px; padding: 15px; margin-bottom: 15px; }
        .mobile-card-row { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 0.95rem; }
      `}</style>
                ...
                {filteredShipments.length > 0 ? filteredShipments.map((item) => (
                    <CTableRow key={item.id}>
                        ...
                        <CTableDataCell className="text-end">
                            <CButton color="link" className="p-1 text-white" onClick={() => openEditModal(item)}><CIcon icon={cilPencil} /></CButton>
                        </CTableDataCell>
                    </CTableRow>
                )) : (
