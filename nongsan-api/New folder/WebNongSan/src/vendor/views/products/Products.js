...
const Products = () => {
  ...
  const getNumericStock = (stockValue) => {
  ...
  const outOfStockProducts = products.filter((item) => getInventoryState(item).key === 'out');
  const lowStockProducts = products.filter((item) => getInventoryState(item).key === 'low');
  ...
  .inventory-warning-card.out {
    background: #fff5f5;
    border-color: #fecaca;
  }

  .inventory-warning-card.low {
    background: #fffbeb;
    border-color: #fde68a;
  }

  .inventory-warning-card-header {
    display: flex;
    align-items: center;
  ...
  .inventory-warning-chip.out {
    color: #991b1b;
    background: #fee2e2;
  }

  .inventory-warning-chip.low {
    color: #92400e;
    background: #fef3c7;
  }
          ...
          {loading ? (
            ...
            <>
              {(outOfStockProducts.length > 0 || lowStockProducts.length > 0) && (
                <div className="inventory-warning-panel">
                  <div className="inventory-warning-title d-flex align-items-center gap-2">
                    <CIcon icon={cilWarning} /> Cảnh báo tồn kho
                  </div>
                  <div className="inventory-warning-grid">
                    {outOfStockProducts.length > 0 && (
                      <div className="inventory-warning-card out">
                        <div className="inventory-warning-card-header">
                          <div className="fw-semibold text-danger">Sản phẩm hết hàng</div>
                          <span className="inventory-warning-chip out">{outOfStockProducts.length} SP</span>
                        </div>
                        <ul className="inventory-warning-list mb-0">
                          {outOfStockProducts.slice(0, 5).map((item) => (
                            <li key={`out-${item.id}`}>{item.name}</li>
                          ))}
                          {outOfStockProducts.length > 5 && <li>...và {outOfStockProducts.length - 5} sản phẩm khác</li>}
                        </ul>
                      </div>
                    )}

                    {lowStockProducts.length > 0 && (
                      <div className="inventory-warning-card low">
                        <div className="inventory-warning-card-header">
                          <div className="fw-semibold text-warning">Sắp hết hàng (≤ {LOW_STOCK_THRESHOLD})</div>
                          <span className="inventory-warning-chip low">{lowStockProducts.length} SP</span>
                        </div>
                        <ul className="inventory-warning-list mb-0">
                          {lowStockProducts.slice(0, 5).map((item) => (
                            <li key={`low-${item.id}`}>{item.name} (còn {getNumericStock(item.stock)} {item.unit || ''})</li>
                          ))}
                          {lowStockProducts.length > 5 && <li>...và {lowStockProducts.length - 5} sản phẩm khác</li>}
                        </ul>
                      </div>
                    )}
                  </div>
                ...
                <CTable hover responsive className="table-green-custom mb-0">
                  ...
                  <CTableBody>
                    {filteredProducts.map((item) => {
                        ...
                        const stockClass = inventoryState.key === 'out' ? 'out' : inventoryState.key === 'low' ? 'low' : '';

                        return (
                          <CTableRow
                            key={item.id}
                            className={`${isBanned ? 'bg-danger bg-opacity-10' : ''} ${inventoryState.key === 'out' ? 'row-out-stock' : inventoryState.key === 'low' ? 'row-low-stock' : ''}`.trim()}
                          >
                            <CTableDataCell>
                            ...
                            <CTableDataCell>
                              <span className={`stock-value ${stockClass}`}>{getNumericStock(item.stock)}</span>
                            </CTableDataCell>
                            ...
                            <CTableDataCell>
                              <span className={`badge-status ${inventoryState.badgeClass}`}>
              ...
              <div className="d-block d-md-none">
                 {filteredProducts.map(item => {
                   const inventoryState = getInventoryState(item);
                   return (
                   <div key={item.id} className={`mobile-product-card ${inventoryState.key === 'out' ? 'row-out-stock' : inventoryState.key === 'low' ? 'row-low-stock' : ''}`}>
                         <div className="d-flex justify-content-between mb-2">
                             <span className="fw-bold text-dark">{item.name}</span>
                       <div className="d-flex gap-1">
                         {item.is_banned == 1 && <CBadge color="danger">Bị Cấm</CBadge>}
                         <CBadge color={inventoryState.badgeColor}>{inventoryState.label}</CBadge>
                       </div>
                         </div>
                         <div className="small text-secondary mb-1">Xuất xứ: {item.origin}</div>
                     <div className="small text-muted-custom mb-1">Tồn kho: {getNumericStock(item.stock)} {item.unit}</div>
                        ...
                        <CRow>
                            ...
                            <CCol md={7} className="ps-md-4">
                                ...
                                <div className="d-flex gap-2 mb-3">
                                  <CBadge color={getInventoryState(viewProduct).badgeColor} shape="rounded-pill">{getInventoryState(viewProduct).label}</CBadge>
                                ...
                                <CRow>
                                    ...
                                    <CCol xs={6}><div className="detail-label">Kho hiện tại</div><div className="detail-value">{getNumericStock(viewProduct.stock)} {viewProduct.unit}</div></CCol>
                                </CRow>
