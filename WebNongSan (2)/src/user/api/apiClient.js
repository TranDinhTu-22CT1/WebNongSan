...
export const authAPI = {
  register: (email, password, name, phone) =>
    apiCall('/register.php', 'POST', { email, password, name, phone }, { withAuth: false }),

  registerInit: (name, email, password, role = 'customer') =>
    apiCall('/register_init.php', 'POST', { name, email, password, role }, { withAuth: false }),

  verifyRegister: (token, otpInput) =>
    apiCall('/verify_register.php', 'POST', { token, otp_input: otpInput }, { withAuth: false }),

  requestPasswordResetOtp: (email) =>
    apiCall('/handle_customers.php?action=forgot_password_init', 'POST', { email }, { withAuth: false }),

  verifyPasswordResetOtp: (token, otpInput) =>
    apiCall('/handle_customers.php?action=verify_reset_otp', 'POST', { token, otp_input: otpInput }, { withAuth: false }),

  resetPassword: (token, password) =>
    apiCall('/handle_customers.php?action=reset_password', 'POST', { token, password }, { withAuth: false }),
  
  login: (email, password, rememberMe = false) =>
    apiCall('/login.php', 'POST', { email, password, remember_me: rememberMe }, { withAuth: false }),
  
  getProfile: async () => {
    const user = getStoredUser() || {};
    if (!user.id && !user.uid) throw new Error('Không tìm thấy thông tin người dùng');
    const response = await apiCall(`/get_profile.php?id=${encodeURIComponent(user.id || user.uid)}`, 'GET');
  ...
  updateProfile: (email, name, phone, address, avatarFile) => {
    ...
    formData.append('id', user.id || user.uid);
  ...
  logout: () => {
    const user = getStoredUser() || {};
    if (!user.id && !user.uid) return Promise.resolve({ status: 'success' });
    return apiCall(`/logout.php?id=${encodeURIComponent(user.id || user.uid)}`, 'POST');
  }
};
...
export const productsAPI = {
  ...
  search: async (query) => {
    ...
    try {
      // Gọi trực tiếp xuống API PHP để tìm kiếm
      const payload = await apiCall(`/handle_products.php?action=search&keyword=${encodeURIComponent(keyword)}`, 'GET', null, { withAuth: false });
      return unwrapListPayload(payload).map(normalizeProduct);
    } catch (error) {
      console.warn('Backend chưa hỗ trợ action=search, chuyển về tìm kiếm cục bộ:', error);
      
      // Dự phòng: Nếu API lỗi (do backend chưa code action=search), vẫn giữ nguyên tính năng lọc ở frontend như cũ
      const products = await productsAPI.getAll();
...
// Orders API (BỎ {withAuth: false} ĐỂ GỬI LẠI TOKEN)
...
export const contactAPI = {
  submit: (name, email, message) =>
    apiCall('/message.php?action=submit_contact', 'POST', { name, email, message }, { withAuth: false }),
};
...
// Payments API
// User API - Dùng để lấy danh sách người dùng/admin
export const userAPI = {
  getAll: async () => {
    // Gọi đến file xử lý user của bạn (thông thường là user_status.php hoặc handle_users.php)
    const payload = await apiCall('/user_status.php?action=get_all_users', 'GET');
    const items = unwrapListPayload(payload);
    ...
    return items.map((item) => ({
      ...
      status: item.status || (item.is_online == 1 ? 'active' : 'Offline'),
    }));
  },
};