...
import { 
  Send, MessageSquare, Image as ImageIcon, X,
...
const SupportChat = () => {
    ...
    try {
      ...
      const mergedItems = allUsers.map(user => {
        ...
        return {
          ...
          last_update_time: existingConv ? (existingConv.updated_at || existingConv.id) : 0 
        };
      });

      // Lọc bỏ chính mình
      let filteredItems = mergedItems.filter(i => String(i.partner_id) !== String(adminId));

      // Sắp xếp: Ưu tiên tin nhắn chưa đọc -> Từng nhắn tin -> Thời gian mới nhất
  ...
  const getAvatarUrl = (user) => {
    ...
    const roleColors = { admin: 'ef4444', customer: '3b82f6' };
    const color = roleColors[user?.role] || '64748b';
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'U')}&background=${color}&color=fff&bold=true`;
  };
