...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        setCurrentPage(1); // Chuyển về trang 1 để xem danh mục vừa thêm mới
  ...
  // Trích xuất danh mục cho trang hiện tại
  ...
  // Đảm bảo nếu xóa hết phần tử ở trang cuối thì lùi về trang trước
  useEffect(() => {
    if (currentCategories.length === 0 && currentPage > 1) {
      setCurrentPage(currentPage - 1);
    }
  }, [currentCategories.length, currentPage]);
    ...
    <div style={s.wrapper}>
      ...
      <div style={s.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ ...s.title, marginBottom: 0 }}>Danh Sách Danh Mục</h2>
          <span style={{ fontSize: '14px', fontWeight: '600', color: '#64748b', background: '#f1f5f9', padding: '6px 12px', borderRadius: '20px' }}>
            Tổng cộng: <strong style={{ color: '#3b82f6' }}>{categoriesList.length}</strong>
          </span>
        </div>
      ...
      <style>{`
        ...
        .page-btn { min-width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border-radius: 10px; border: 1px solid #e2e8f0; background: #fff; color: #64748b; font-weight: 700; cursor: pointer; transition: 0.2s; font-size: 14px; }
        .page-btn:hover:not(:disabled) { background: #f8fafc; color: #3b82f6; border-color: #cbd5e1; }
