...
const Home = () => {
  ...
  const API_BASE = (import.meta.env.VITE_API_BASE_URL || 'http://localhost/nongsan-api').replace(/\/$/, '');

  useEffect(() => {
    const loadBanners = async () => {
      try {
        const response = await fetch(`${API_BASE}/banner.php?action=list`);
        const payload = await response.json();
        if (payload?.status === 'success') {
          setBannerData({
            system: Array.isArray(payload?.data?.system) ? payload.data.system : [],
            promo: Array.isArray(payload?.data?.promo) ? payload.data.promo : [],
          });
        }
      } catch (error) {
        console.error('Cannot load banners from API:', error);
      }
    };
  ...
  const promoHeroBanners = [...bannerData.promo]
    ...
    .map((item, index) => ({
      url: normalizeImagePath(item.image_path),
  ...
  const heroBannerImage = normalizeImagePath(
    bannerData.system.find((item) => item.banner_key === 'user_hero')?.image_path || ''
  );
      ...
      <div className="home-container">
        ...
        <div className="promo-banners">
          ...
          <div className="promo-card promo-orange">
            ...
            <div className="promo-img-bg" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1613478223719-2ab802602423?auto=format&fit=crop&w=600&q=80')" }}></div>
          </div>
        </div>
